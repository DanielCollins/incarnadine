...
impl Floor {
    ...
    pub fn new(window: &types::Display) -> Self {
        ...
        let buffer = glium::vertex::VertexBuffer::new(window, &[
            Vertex { position: [-1.0,  0.0, 1.0], normal: [0.0, 1.0, 0.0], tex_coords: [0.0, 1.0] },
            Vertex { position: [ 1.0,  0.0, 1.0], normal: [0.0, 1.0, 0.0], tex_coords: [1.0, 1.0] },
            Vertex { position: [-1.0, 0.0, -1.0], normal: [0.0, 1.0, 0.0], tex_coords: [0.0, 0.0] },
            Vertex { position: [ 1.0, 0.0, -1.0], normal: [0.0, 1.0, 0.0], tex_coords: [1.0, 0.0] },
        ]).unwrap();
    ...
    pub fn render(self: &Self, frame_buffer: &mut glium::Frame,
        ...
        frame_buffer.draw(&self.buffer, &glium::index::NoIndices(strip),
                          &library.lit_texture, &uniforms, &params).unwrap();
    }
}
